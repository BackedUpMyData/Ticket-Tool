...
module.exports = async (bot, reaction, user) => {
  ...
  if(message.embeds.length === 1 && message.embeds[0].title === 'Ticket System' && message.embeds[0].description === 'React with 🎟️ to create a ticket.'){
    if(reaction.emoji.name === "🎟️"){
      if(!message.guild.channels.cache.find(c => c.name === `ticket-${usr[0]}${usr[1]}${usr[2]}${usr[3]}`)){
        ...
        if(!role) {
          message.guild.roles.create({data:{name: "Ticket Support", permissions: 0}, reason: 'Staff need this role to see tickets.'});
        ...
        .setDescription(`An user has opened a ticket and is waiting for their request to be handled.`)
  ...
  if(message.embeds.length === 1 && message.embeds[0].title === '🎟️ | Ticket Ended' && message.embeds[0].description === `React with \\🗑️ to close the ticket.`){
    if(reaction.emoji.name === "🗑️"){
      if(user.id === db.get(`ticket.${message.channel.name}.user`)){
        ...
        .setColor(color.none)
        .setDescription(`Ticket will be closed.`)
        .setTimestamp()
        .setFooter(`Ticket System`, bot.user.displayAvatarURL())
